    ...
    <div className="bg-[#0C0E16] border border-white/15 rounded-3xl overflow-hidden shadow-2xl space-y-0">
      ...
      <div className="bg-[#14171E] border-b border-[#1E2228] px-6 py-2.5 flex flex-wrap justify-between items-center gap-4 text-xs font-mono">
        ...
        <button
          ...
          disabled={isLoading}
          className="px-3 py-1 rounded bg-[#1A202C] hover:bg-[#C29244] hover:text-[#07080A] text-gray-200 border border-[#2D3748] transition flex items-center gap-1.5 cursor-pointer disabled:opacity-50"
        >
        ...
        <div className="lg:col-span-8 bg-[#090A0C] border border-[#1E2228] rounded-xl p-5 relative flex flex-col justify-between min-h-[400px] overflow-hidden select-none">
          ...
          <div className="flex flex-wrap justify-between items-center gap-2 z-10">
            <div className="flex items-center gap-4 text-xs font-mono">
              <span className="flex items-center gap-1.5 text-gray-300">
                <span className="w-3 h-0.5 bg-[#C29244]"></span> 2020–2026 Observations
              </span>
          ...
          <div className="relative w-full h-[250px] my-2">
            <svg className="w-full h-full" viewBox="0 0 600 250" preserveAspectRatio="none">
              ...
              <line x1={getX(3)} y1="15" x2={getX(3)} y2="215" stroke="#C29244" strokeWidth="1.5" strokeDasharray="3,3" />
              <text x={getX(3) + 5} y="26" fill="#C29244" fontSize="10" fontFamily="monospace" fontWeight="bold">2026 (NOW)</text>
              ...
              {selectedMetric === 'crack' && (
                <>
                  ...
                  />
                  {[0, 1, 2, 3].map(i => (
                    <circle key={i} cx={getX(i)} cy={getYCrack(timeSeriesData[i]?.crackLength || 20)} r={i === 3 ? 6 : 4} fill="#C29244" onMouseEnter={() => setHoveredIndex(i)} />
                  ))}
                  {[4, 5, 6, 7].map(i => (
              ...
              {selectedMetric === 'moisture' && (
                <>
                  ...
                  />
                  {[0, 1, 2, 3].map(i => (
                    <circle key={i} cx={getX(i)} cy={getYMoisture(timeSeriesData[i]?.moisture || 10)} r={i === 3 ? 6 : 4} fill="#C29244" onMouseEnter={() => setHoveredIndex(i)} />
                  ))}
                  {[4, 5, 6, 7].map(i => (
                    <g key={i} onMouseEnter={() => setHoveredIndex(i)}>
              ...
              {timeSeriesData.map((pt, i) => (
                <text
                  ...
                  y="235"
                  textAnchor="middle"
                  fill={i === 3 ? '#C29244' : i >= 4 ? '#A7A296' : '#6E6A60'}
                  fontSize="11"
          ...
          <div className="bg-[#121418] border border-[#232A38] p-3 rounded-xl flex flex-wrap justify-between items-center gap-3 text-xs font-mono">
            <div className="flex items-center gap-2">
              <span className="text-[#C29244] font-bold">[{activePoint?.year}] Epoch Telemetry:</span>
            ...
            <div className="flex items-center gap-4 text-gray-300">
              {activePoint?.type === 'forecast' ? (
                ...
                <span>Recorded Status: <strong className="text-[#C29244]">{selectedMetric === 'health' ? `Health ${activePoint?.health}/100` : selectedMetric === 'crack' ? `${activePoint?.crackLength} cm` : `${activePoint?.moisture}% moisture`}</strong></span>
              )}
            </div>
          </div>
